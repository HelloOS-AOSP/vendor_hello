...
func init() {
	android.RegisterModuleType("hello_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = helloExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = helloExpandVariables(ctx, dep_file)
	...
	cmd := helloExpandVariables(ctx, String(g.properties.Cmd))
