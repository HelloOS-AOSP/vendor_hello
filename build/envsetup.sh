function __print_hello_functions_help() {
cat <<EOF
Additional HelloOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- hellogerrit:     A Git wrapper that fetches/pushes patch from/to HelloOS Gerrit Review.
- hellorebase:     Rebase a Gerrit change and push it again.
- helloremote:     Add git remote for HelloOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for HelloOS Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/hello/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch hello_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/hello-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.hello.device | grep -q "$HELLO_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $HELLO_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function helloremote()
{
    ...
    git remote rm hello 2> /dev/null
    ...
    local HELLO_USER=$(git config --get review.review.helloos.org.username)
    if [ -z "$HELLO_USER" ]
    then
        git remote add hello ssh://review.helloos.org:29418/$PFX$PROJECT
    else
        git remote add hello ssh://$HELLO_USER@review.helloos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'hello' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/HelloOS-AOSP/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.hello.device | grep -q "$HELLO_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        helloremote
        git push hello HEAD:refs/heads/'$1'
    fi
    '
}

function hellogerrit() {
    ...
    local user=`git config --get review.review.helloos.org.username`
...
function hellorebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "HelloOS Gerrit Rebase Usage: "
        echo "      hellorebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.helloos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.hello.device | grep -q "$HELLO_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/hello/build/tools/repopick.py $@
}
